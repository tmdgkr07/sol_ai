...
def _run_analysis_for_ui(
    file_path: str | None, n_clusters: int | float
) -> tuple[Any, ...]:
    """Gradio 분석 버튼 콜백."""
    if not file_path:
        raise gr.Error("CSV 파일을 업로드해 주세요.")

    try:
        analysis = build_analysis(file_path, int(n_clusters))
    except Exception as error:
    ...
    default_cluster = cluster_choices[0]
    cluster_status, cluster_table = filter_cluster_opinions(
        default_cluster, search_state
    )

    default_query = "취업지원"
    exact_results = exact_keyword_search(
        default_query, search_state, top_k=5
    )
    ...
    recommendation_text = (
        str(recommended_k) if recommended_k is not None else "계산 불가"
    )
    status = (
        "### 분석 완료\n"
        f"- 사용 인코딩: **{stats.get('encoding', '-')}**\n"
        f"- 정제 전/후: **{stats.get('before_rows', 0):,} → "
        f"{stats.get('after_rows', 0):,}개**\n"
        f"- 선택한 k: **{analysis['kmeans'].n_clusters}**\n"
        f"- Silhouette 추천 k: **{recommendation_text}**\n"
        f"- Gemini Issue/Root Cause/Action: "
        f"**{analysis['gemini_summary_status']}**\n"
        f"- PCA 설명 분산: "
    ...
    dropdown_update = gr.Dropdown(
        choices=[
            (f"Cluster {cluster_id}", cluster_id)
            for cluster_id in cluster_choices
        ],
...
def _filter_cluster_for_ui(
    cluster_id: int | float | str | None,
    search_state: dict[str, Any] | None,
) -> tuple[str, pd.DataFrame]:
    try:
        return filter_cluster_opinions(cluster_id, search_state)
    except Exception as error:
        raise gr.Error(str(error)) from error


def _run_search_for_ui(
    query: str,
    top_k: int | float,
    search_state: dict[str, Any] | None,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    if not search_state:
        raise gr.Error("먼저 CSV 분석을 실행해 주세요.")
    try:
        exact_results = exact_keyword_search(
            query, search_state, int(top_k)
        )
        semantic_results = semantic_search(
            query, search_state, int(top_k)
        )
...
def create_app() -> gr.Blocks:
    """배포용 Gradio 애플리케이션을 생성한다."""
    with gr.Blocks(title="의견 분석 대시보드") as demo:
        analysis_state = gr.State(value=None)

        gr.Markdown(
            "# 의견 분석 대시보드\n"
            "CSV의 `text` 컬럼을 임베딩하고 클러스터별 핵심 주제와 "
            "대표 의견을 분석합니다. `GEMINI_API_KEY`가 설정된 경우 "
            "클러스터 의견 표본이 Gemini API로 전송되어 "
            "Issue / Root Cause / Action 요약이 자동 생성됩니다."
        )
        ...
        analysis_status = gr.Markdown("CSV 파일을 업로드해 주세요.")

        with gr.Tabs():
            with gr.Tab("클러스터 요약"):
                cluster_summary_output = gr.Dataframe(
                    value=EMPTY_CLUSTER_SUMMARY,
                    interactive=False,
                    wrap=True,
                    show_search="filter",
                    buttons=["fullscreen", "copy"],
                    label=(
                        "클러스터별 의견 수·키워드·대표 의견·"
                        "Issue/Root Cause/Action"
                    ),
                )
                representative_output = gr.Dataframe(
                    value=EMPTY_REPRESENTATIVES,
                    interactive=False,
                    wrap=True,
                    buttons=["fullscreen", "copy"],
                    label="클러스터별 대표 의견 Top 3",
                )

            with gr.Tab("Gemini 요약"):
                gr.Markdown(
                    "Gemini 3.5 Flash-Lite가 클러스터별 의견 표본을 "
                    "근거로 Issue / Root Cause / Action을 생성합니다."
                )
                gemini_summary_output = gr.Dataframe(
                    value=EMPTY_GEMINI_SUMMARY,
                    interactive=False,
                    wrap=True,
                    show_search="filter",
                    buttons=["fullscreen", "copy"],
            ...
            with gr.Tab("PCA Topic Map"):
                pca_plot = gr.Plot(label="PCA Topic Map")

            with gr.Tab("UMAP Topic Map"):
                umap_plot = gr.Plot(label="UMAP Topic Map")
            ...
            with gr.Tab("클러스터 의견"):
                ...
                cluster_count_text = gr.Markdown(
                    "분석 후 클러스터를 선택할 수 있습니다."
                )
                cluster_opinion_table = gr.Dataframe(
                    value=EMPTY_CLUSTER_OPINIONS,
                    interactive=False,
                    wrap=True,
                    show_search="search",
                    buttons=["fullscreen", "copy"],
            ...
            with gr.Tab("의견 검색"):
                ...
                with gr.Row():
                    exact_output = gr.Dataframe(
                        value=EMPTY_EXACT_RESULTS,
                        interactive=False,
                        wrap=True,
                        label="Exact Keyword Search",
                    )
                    semantic_output = gr.Dataframe(
                        value=EMPTY_SEMANTIC_RESULTS,
                        interactive=False,
                        wrap=True,
        ...
        analyze_button.click(
            fn=_run_analysis_for_ui,
            inputs=[csv_file, n_clusters_input],
            outputs=[
                analysis_status,
        ...
        # 사용자가 직접 선택했을 때만 실행한다. 분석 콜백이 dropdown 값을
        # 갱신할 때 중복 실행되는 것을 피하기 위해 change 대신 input을 쓴다.
        cluster_dropdown.input(
            fn=_filter_cluster_for_ui,
            inputs=[cluster_dropdown, analysis_state],
            outputs=[cluster_count_text, cluster_opinion_table],
            show_progress="hidden",
        )

        search_button.click(
            fn=_run_search_for_ui,
            inputs=[query_input, top_k_input, analysis_state],
            outputs=[exact_output, semantic_output],
        )
        query_input.submit(
            fn=_run_search_for_ui,
            inputs=[query_input, top_k_input, analysis_state],
